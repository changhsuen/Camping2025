...
// 使用你的 SVG 檔案 - 16px 大小
const statusIcons = {
    'status-none': './icons/Not started.svg',
    'status-partial': './icons/Incomplete.svg', 
    'status-complete': './icons/Complete.svg'
};
...
function createStatusIndicator(itemId, responsiblePersons) {
    ...
    svgImg.src = statusIcons[statusClass];
    svgImg.width = 16;
    svgImg.height = 16;
    svgImg.alt = statusClass;
    
    statusIndicator.appendChild(svgImg);
...
function updateStatusIndicators() {
    ...
    items.forEach(item => {
        ...
        if (statusContainer) {
            ...
            // 更新狀態 SVG
            const statusClass = getStatusClass(itemId, responsiblePersons);
            const svgImg = statusIndicator.querySelector('img');
            if (svgImg) {
                svgImg.src = statusIcons[statusClass];
                svgImg.alt = statusClass;
            }
